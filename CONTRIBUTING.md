...
_*If you don't have a GPU, you can use alternative LLM providers (currently supported: Groq, OpenAI, Mistral, Claude, Gemini)_
...
- `MISTRAL_API_KEY`: your [MISTRAL_API_KEY](https://console.mistral.ai/api-keys/), required if you select `mistral` as `LLM_PROVIDER`
- `MISTRAL_MODEL`: the model tag in [Mistral supported models](https://docs.mistral.ai/getting-started/models/) that will be used for the API.
...
- `GEMINI_API_KEY`: your [GEMINI_API_KEY](https://aistudio.google.com/app/apikey), required if you select `gemini` as `LLM_PROVIDER`
- `GEMINI_MODEL`: the model tag in [Gemini supported models](https://ai.google.dev/gemini-api/docs/models/gemini) that will be used for the API.
